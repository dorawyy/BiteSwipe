...
// Send a notification to the desired user
async function sendNotification(fcmToken: string, title: string, body: string, data?: { [key: string]: string }) {
    ...
    try {
        const response = await getMessaging().send(message); // Use getMessaging() to prevent crashes
        console.log("Notification Sent:", response);
    } catch (error) {
        console.error("ERROR: notification not sent, ", error);
        console.error("Firebase Token associated with error: ", fcmToken)
    }
}

export { getMessaging, sendNotification };
