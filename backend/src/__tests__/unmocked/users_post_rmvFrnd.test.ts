...
describe('POST /users/:email/removeFriend', () => {
    beforeAll(() => {
        const app = createApp();
        agent = supertest(app);
    });
    ...
    test('POST /users/:email/removeFriend', async () => {
        const userData1 = {
        ...
        const response2 = await agent
            .post('/users/test1@example.com/removeFriend')
            .send({
                userEmail: ' ',
                friendEmail: ' '
            });
    ...
    test('POST  /Users/:email/removeFriend', async () => {
        ...
        const response3 = await agent 
            .post('/users/test2@example.com/removeFriend')
            .send({
    ...
    test('POST /users/:email/friendRequest Success and than Fail', async () => {
        ...
        expect(response4.body.status).not.toBeNull();
